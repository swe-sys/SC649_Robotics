...
P = np.diag([0.5, 0.5, 0.5])  # Some reasonable initial values - State Covariance
...
def callback(data):
    ...
    residual = Y_measured - predicted_measurement #  residual = z - h(mu_bar)
    ...
    # # print(PH_T, 'PH')
    S = np.matmul(H, PH_T) + R  # Bracket Calculation
